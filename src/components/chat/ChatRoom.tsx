...
import { initializeMemoryUser, retrieveMemoryContext, createConversation, saveMessage, getConversationMessages } from '@/services/memoryService';
...
const ChatRoom: React.FC<ChatRoomProps> = ({
  ...
  const handleSendMessage = async (content: string) => {
    ...
    if (isNewChat) {
      try {
        ...
        if (memoryEnabled && memoryUser) {
          await createConversation(memoryUser.id, newChatTitle, selectedFeature || undefined);
        }
      } catch (error: any) {
        toast({
    ...
    try {
      ...
      const aiMessage = await sendMessage(currentChatId!, response.data.response, 'assistant');
  ...
  return <div className="flex flex-col h-full bg-[#F8F8F4]">
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-3xl w-full">
            <Button variant="outline" className="h-auto p-4 flex flex-col items-start text-left" onClick={() => handleSendMessage("Analyze crop yield factors")}>
            ...
            <Button variant="outline" className="h-auto p-4 flex flex-col items-start text-left" onClick={() => handleSendMessage("Create a business forecast")}>
            ...
            <Button variant="outline" className="h-auto p-4 flex flex-col items-start text-left" onClick={() => handleSendMessage("Calculate ROI for new equipment")}>
            ...
            <Button variant="outline" className="h-auto p-4 flex flex-col items-start text-left" onClick={() => handleSendMessage("Compare crop performance")}>
    ...
    <>
          ...
          <ScrollArea className="flex-1 px-4 py-6" ref={scrollAreaRef}>
            <div>
              {messages.map(message => <ChatMessage key={message.id} sender={message.role} content={message.content} timestamp={new Date(message.created_at!).toLocaleTimeString()} />)}
